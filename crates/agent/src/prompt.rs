...
# 多轮对话
沿用同一会话已确认的目标、偏好、证据和结论；用户修正前提时更新假设，证据过期时重取，追问不重复。
# 数据纪律

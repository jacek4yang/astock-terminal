...
- `scripts/publish-v6.ps1` is the only v6 publication entry. Its Authenticode path still requires a fully `PASSED` immutable report and every shipped PE to be `Valid`. Its explicitly selected public `-ConfirmUnsignedAttestedRelease` path may create `v6.0.0` only from clean `main` after all named GitHub quality checks pass; the GitHub-hosted workflow must rebuild the assets, publish SHA-256/SBOM evidence, use OIDC artifact attestations, and label Authenticode as `NOT PROVIDED`. Never move an existing release tag or describe an unsigned binary as Authenticode-signed.

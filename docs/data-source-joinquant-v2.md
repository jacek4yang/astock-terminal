...
- 生产实现只将账号凭据保存到 Windows Credential Manager；“本地”不包括 SQLite/`kv`、配置文件、环境变量、命令行或 React 状态。Engine 仅在认证请求期间持有私有内存副本，并禁止凭据进入日志与诊断证据。
...
2. **做成可选 provider**：设置页的受控输入直接写入 Windows Credential Manager；无凭证/登录失败时整体禁用，不影响现有免费源，也不得回退到 SQLite、环境变量或明文配置。

...
每个来源拥有独立的节流、重试和健康状态。瞬时错误使用指数退避；认证、许可和参数错误不会高频重试。单个来源失败不阻断其他来源，结果中保留具体缺口。SEC 只有在 Windows Credential Manager 的 `provider-sec-user-agent` 槽配置 Fair Access 身份且用户提供 CIK 时才访问；World Bank 可直接同步官方指标。

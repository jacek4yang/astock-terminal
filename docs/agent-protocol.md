...
页面切换不会卸载 Agent 主视图。会话、任务、事件、Effect、检查点和报告保存在 Engine 数据库中，React 状态只用于当前视图；会话中的任务快照只能展示，不能作为可执行恢复状态。打开历史或重试前，Renderer 必须读取 `agent.task.load`，核对 `task_id` 与 `accepted_seq` 后才允许继续，读取失败时按钮保持关闭。会话支持列表、服务端搜索、新建、重命名、软删除、恢复，以及从指定用户消息或最新持久化检查点分支为新研究。检查点分支由 Engine 再次核对源会话、durable task 与 checkpoint 三者的 `task_id/accepted_seq`，并记录可审计的 `branchOrigin`；新分支不会复制旧可执行状态，而是保留消息与明确任务边界、清空 task/effect/checkpoint，再重新取得当前数据。搜索最多返回协议页大小范围内的结果，不会把全部历史搬入 Renderer。

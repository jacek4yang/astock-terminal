...
结构化响应最多有三次有界尝试，使用已验证的快速、主分析和复核模型接力。MiniMax 返回的 `reasoning_details` 或 `<think>` 私有推理不会写入用户会话、工具活动或最终报告；Worker 只提取最终 JSON 或可见文本。工具活动只公开输入摘要、状态、耗时、缓存命中和证据，不展示私有推理链。
...
该门禁不等价于逐一复算报告中的所有金额、比例、日期和目标价。逐字段数值复现、跨源重大冲突阻断和每条结论到 `evidence_id` 的完整闭包仍是发布前必须补齐并通过的门禁；在实际通过之前应标记为 `NOT VERIFIED`，不得写成已经形式证明或生产验证完成。
